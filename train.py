...
from model import BrainEncoder, NeuroFlow
from modules.flow import RealNVP
...
parser.add_argument('--model_save_path', type=str, default='train_logs/demo_single_subject/sub01_v4', help='path to save results')
...
parser.add_argument('--fmri_encoder', type=str, default='NeuroFlow', help='type of brainnet', choices=['NeuroFlow'])
parser.add_argument('--batch_size', type=int, default=32, help='batch size for training')
parser.add_argument('--num_epochs', type=int, default=240, help='number of epochs of training')
parser.add_argument('--seed', type=int, default=42)
parser.add_argument('--resume', type=str, default='./train_logs/demo_single_subject/sub01_v4/last.pth', help='path to checkpoint to resume training')
...
if fmri_encoder == 'NeuroFlow':
    voxel2emb = NeuroFlow(in_dim=num_voxels, hidden_dim=1024, out_dim=feat_dim, num_latents=256)
else:
    raise ValueError("The fmri encoder is not implemented.")
voxel2emb.to(device)

real_nvp = RealNVP([
    AffineTransform("left", input_size=256 * 1024, n_hidden=2, hidden_size=64),
    AffineTransform("right", input_size=256 * 1024, n_hidden=2, hidden_size=64),
    AffineTransform("left", input_size=256 * 1024, n_hidden=2, hidden_size=64),
    AffineTransform("right", input_size=256 * 1024, n_hidden=2, hidden_size=64),
    AffineTransform("left", input_size=256 * 1024, n_hidden=2, hidden_size=64),
    AffineTransform("right", input_size=256 * 1024, n_hidden=2, hidden_size=64)
...
opt_grouped_parameters = [
    ...
    {'params': [p for n, p in real_nvp.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': 1e-2},
    {'params': [p for n, p in real_nvp.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}
]
...
if lr_scheduler_type == 'linear':
    ...
    print('total_steps______', total_steps)
...
progress_bar = tqdm(range(epoch, num_epochs))
...
for epoch in progress_bar:
    ...
    for train_i, (voxel, image) in enumerate(train_dl):
        ...
        #flow
        image_features = emb_image.reshape(emb_image.size(0),-1)
        fMRI_features = emb_voxel.reshape(emb_voxel.size(0),-1)
        fMRI_features, log_det = real_nvp.flow(image_features)
        reconstructed_image = real_nvp.invert_flow(fMRI_features)
        # Loss computation
        nll_loss = real_nvp.nll(image_features)
        jacobian_loss = real_nvp.jacobian_clamping(image_features)
        loss_flow = nll_loss + jacobian_loss
