...
from modules.BFA import FourierUnit_modified, ChannelAggregationFFN
...
class BrainEncoder(nn.Module):
    def __init__(self):
        ...
        self.clip = CLIPVisionModel.from_pretrained('huggingface/models--openai--clip-vit-large-patch14/snapshots/32bd64288804d66eefd0ccbe215aa642df71cc41')      
...
class NeuroFlowCross(nn.Module):
    def __init__(self, hidden_dim=1024, out_dim=1024, num_latents=256, use_token=False, use_norm=False, act_first=False):
        ...
        self.uffc = FourierUnit_modified(in_channels=256, out_channels=256)
        self.ca = ChannelAggregationFFN(embed_dims=256)
    ...
    def forward(self, x, modal='fmri1'):
        ...
        x2 = self.sigmod(x2)
        x = x1*x2
        x = x.view(x.size(0), x.size(1), 1024) #(32,256,1024)
        return x
